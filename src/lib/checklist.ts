  ...
  <body>
    ...
    <script>
      ...
      function currentPageOpenScript() {
        return (
          "try{if(location.protocol==='http:'||location.protocol==='https:')localStorage.setItem('checklist_auto:homeUrl',location.origin+'/');}catch(e){}" +
          "function __goHome(){var u='/';try{u=localStorage.getItem('checklist_auto:homeUrl')||u;}catch(e){}try{if(u==='/'&&(location.protocol==='http:'||location.protocol==='https:'))u=location.origin+'/';}catch(e){}location.href=u;}" +
          ...
          "function __replaceWithBlobHtml(html){var b=new Blob([html],{type:'text/html;charset=utf-8'});location.href=URL.createObjectURL(b);}" +
          ...
          "function __readonlySnapshotHtml(){var c=document.documentElement.cloneNode(true);var si=document.querySelectorAll('input,textarea,select');var ci=c.querySelectorAll('input,textarea,select');for(var i=0;i<ci.length;i++){var s=si[i];var d=ci[i];if(!s||!d)continue;if(d.tagName==='TEXTAREA'){d.textContent=s.value||'';}else if(d.tagName==='SELECT'){Array.prototype.forEach.call(d.options,function(o){if(o.value===s.value)o.setAttribute('selected','selected');else o.removeAttribute('selected');});}else{d.setAttribute('value',s.value||'');}}Array.prototype.forEach.call(c.querySelectorAll('select,input,textarea'),function(el){el.setAttribute('disabled','disabled');el.removeAttribute('onclick');});Array.prototype.forEach.call(c.querySelectorAll('script'),function(el){el.remove();});var keep=c.ownerDocument.createElement('script');keep.textContent=\"function __goHome(){var u='/' ;try{u=localStorage.getItem('checklist_auto:homeUrl')||u;}catch(e){}try{if(u==='/'&&(location.protocol==='http:'||location.protocol==='https:'))u=location.origin+'/';}catch(e){}location.href=u;}\";c.body.appendChild(keep);return '<!doctype html>\\\\n'+c.outerHTML;}" +
      ...
      function buildReportListHtml(key, title, emptyText) {
        const script =
          ...
          "function __prepareReportHtml(html){var next=String(html||'');next=next.replace(/<button\\b[^>]*>\\s*홈\\s*<\\/button>/i,'<button type=\"button\" onclick=\"window.__goHome&&window.__goHome()\">홈</button>');if(next.indexOf('function __goHome()')<0){next=next.replace('</body>','<script>function __goHome(){var u=\\'/\\';try{u=localStorage.getItem(\\'checklist_auto:homeUrl\\')||u;}catch(e){}try{if(u===\\'/\\'&&(location.protocol===\\'http:\\'||location.protocol===\\'https:\\'))u=location.origin+\\'/\\';}catch(e){}location.href=u;}<\\/script></body>')}return next;}" +
          ...
          "window.__openSavedReport=function(id){var found=readReports().find(function(r){return r.id===id});if(found&&found.html)__replaceWithBlobHtml(__prepareReportHtml(found.html));};" +
          "render();";
        const body =
          "<div class=\\"top\\"><div><h1>" + title + "</h1><div class=\\"muted\\">이 브라우저에 저장된 결과 리포트</div></div>" +
          "<div class=\\"toolbar noPrint\\"><button type=\\"button\\" onclick=\\"window.__goHome&&window.__goHome()\\">홈</button><button type=\\"button\\" class=\\"primary\\" onclick=\\"window.print()\\">인쇄/PDF 저장</button></div></div>" +
